...
class TOFSensor2Node(Node):
    def __init__(self):
        super().__init__('tof_sensor_2_node')
        ...
        self.publisher = self.create_publisher(Float32, 'tof_distance_2', 10)
        ...
        try:
            ...
            # Power up only Sensor 2, keep Sensor 1 off
            self.xshut1.value = False
            self.xshut2.value = True
            time.sleep(0.3)
            
            # Initialize Sensor 2 (will be at default address 0x29)
            self.sensor = VL53L0X(self.i2c)
            self.get_logger().info("TOF Sensor 2 initialized at address 0x29")
    ...
    def read_distance(self):
        """Read distance from TOF sensor 2."""
        try:
            if self.sensor is not None:
                try:
                    # Keep Sensor 2 powered, Sensor 1 off
                    self.xshut1.value = False
                    self.xshut2.value = True
                    ...
                    print(f"Sensor 2: {distance_cm:6.2f} cm", flush=True)
                except Exception as e:
